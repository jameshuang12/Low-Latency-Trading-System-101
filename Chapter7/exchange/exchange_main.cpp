...
int main(int, char **) {
    ...
    matching_engine = new Exchange::MatchingEngine(&client_requests, &client_responses, &market_update);
    ...
    market_data_publisher = new Exchange::MarketDataPublisher(&market_updates, mkt_pub_iface,
                                                              snap_pub_ip, snap_pub_port, inc_pub_ip, inc_pub_port);
    ...
    while(true) {
        logger->log("%:% %() % Sleeping for a few milliseconds..", __FILE__, __LINE__,
                    __FUNCTION__, Common::getCurrentTImeStr(&time_str));
