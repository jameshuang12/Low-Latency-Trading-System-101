...
namespace Exchange {
    OrderServer::OrderServer(ClientRequestLFQueue *client_request, ClientResponseLFQueue *client_responses,
                             const int &iface, int port) :
        ...
        cid_next_exp_seq_num.fill(1);
        cid_tcp_socket_.fill(nullptr);

        tcp_server_.recv_callback = [this](auto socket, auto rx_time) {
