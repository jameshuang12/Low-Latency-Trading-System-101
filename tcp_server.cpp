...
namespace Common {
    auto TCPServer::addToEpollList(Common::TCPSocket *socket) {
    ...
    auto TCPServer::poll() noexcept -> void {
        ...
        while (have_new_connection) {
            logger_.log("%:% %() % have_new_connection\n", __FILE__, __LINE__, __FUNCTION__, sockaddr_storage addr);
            sockelen_t addr_len = sizeof(addr);
