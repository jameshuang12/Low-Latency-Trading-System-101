...
auto consumeFunction(LFQueue<MyStruct>* lfq) {
    ...
    while(lfq->size()) {
        ...
        lfq->updateReadIndex();
        std::cout << "consumeFunction read elem:" << d->d_[0] << lfq->size() << std::endl;
        std::this_thread::sleep_for(1s);
    }
    std::cout << "consumeFunction existing." << std::endl;
}
...
int main(int, char**){
    ...
    for(auto i = 0; i < 50; ++i){
        ...
        *(lfq.getNextToWriteTo()) = d;
        ...
        std::cout << "main constructed elem:" << d.d_[0] << "," << d.d_[1] << "," << d.d_[2] << "lfq-size:"
        << lfq.size() << std::endl;
